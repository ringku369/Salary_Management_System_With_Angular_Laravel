...
class ApiRetailerController extends Controller
{
  ...
  public function login(Request $request)
  {
    ...
    $credentials1 = ['email'=> $request['username'],'password'=> $request['password'],'level' => 500,'active' => 1];
    $credentials2 = ['email'=> $request['username'],'password'=> $request['password'],'level' => 400,'active' => 1];
    $credentials3 = ['email'=> $request['username'],'password'=> $request['password'],'level' => 200,'active' => 1];
    $credentials4 = ['officeid'=> $request['username'],'password'=> $request['password'],'level' => 200,'active' => 1];

    if ($token = auth()->claims(['role'=>'Admin'])->attempt($credentials1)) {
      return $this->respondWithToken($token);
    }elseif($token = auth()->claims(['role'=>'User'])->attempt($credentials2)){
      return $this->respondWithToken($token);
    }elseif($token = auth()->claims(['role'=>'Retailer'])->attempt($credentials3)){
      return $this->respondWithToken($token);
    }elseif($token = auth()->claims(['role'=>'Retailer'])->attempt($credentials4)){
      return $this->respondWithToken($token);
    }else {
