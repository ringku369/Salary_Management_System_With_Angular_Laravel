...
class ApiAdminController extends Controller
{
  ...
  public function dateWiseSelfDLUser(Request $request)
  {
    ...
    $queryResult = User::select('id','username','name','email','district','profession','address','gender','_lft','_rgt','parent_id','created_at','contact','balance','bmcount',
  ...
  public function dateWiseSelfProfit(Request $request)
  {
    ...
    $queryResult = Profit::with('user','dsponsor','child')
      ...
      ->select('id','user_id','ds_id','ids_id','child_id','amount','status','created_at', DB::raw('(CASE WHEN status = 1 THEN "Membership" WHEN status = 2 THEN "Direct Sponsor " WHEN status = 3 THEN "Reference" WHEN status = 4 THEN "Team Performance" WHEN status = 5 THEN "Generation" WHEN status = 6 THEN "Fund Transfer" ELSE "VizzClub Incentive" END) AS incometype'),
        DB::raw("DATE_FORMAT(created_at,'%Y-%m-%d') as date")
      )
  ...
  public function dateWiseUserBlnUpdate(Request $request)
  {
    $rules  =  array(
      'fdate' => 'required|min:6',
      'todate' => 'required|min:6',
    ...
    $fn_transaction = function() use ($request)
    {
      try
      {
        ...
        // Updated Query
        $query = User::select('id')->get();
        $users = $query->toArray();
        foreach ($users as $key=>$user) {
          
          self::BMtBlnUpdate($user['id']);
          self::AllTypeBlnUpdate($request->fdate, $request->todate, $user['id']);
          self::RankUpdate($user['id']);
          self::dailyIncentiveCount($user['id']);
          //self::OpeningBlnUpdate($request->fdate, $request->todate, $user['id']);
          //self::DSBlnUpdate($request->fdate, $request->todate, $user['id']);
          //self::IDSBlnUpdate($request->fdate, $request->todate, $user['id']);
          //self::MTBlnUpdate($request->fdate, $request->todate, $user['id']);
          //self::GenBlnUpdate($request->fdate, $request->todate, $user['id']);
  ...
  // Binary Matching Balance Update

  private function BMtBlnUpdate($user_id){

    // Matching Count Section

    $count = User::select('id')->where(['position'=>1,'parent_id'=> $user_id])->count();

    if ($count > 0 ) {
      $tuser = User::select('id')->where(['position'=>1,'parent_id'=> $user_id])->first();
      $tid = $tuser->id;
      $queryResult = User::select('id')->descendantsAndSelf($tid)->toFlatTree();
      $results = $queryResult->toArray();
      $lftcount = count($results);
    } else {
      $lftcount = 0;
    }

    $count = User::select('id')->where(['position'=>2,'parent_id'=> $user_id])->count();
    if ($count > 0 ) {
      $tuser = User::select('id')->where(['position'=>2,'parent_id'=> $user_id])->first();
      $tid = $tuser->id;
      $queryResult = User::select('id')->descendantsAndSelf($tid)->toFlatTree();
      $results = $queryResult->toArray();
      $rgtcount = count($results);
    } else {
      $rgtcount = 0;
    }
    ...
    $user = User::select('id','bmcount')->find($user_id);
    $bmcount = $user->bmcount;
    $fbmcount = $mcount - $bmcount;

    if ($fbmcount > 0) {
      //Update Profits
      $data_arr['user_id'] = $user_id;
      $data_arr['amount'] = $fbmcount * 3;
      $data_arr['status'] = 4;
      $data_arr['iscount'] = 0;
      Profit::create($data_arr);
      //Update Profits
      $user->bmcount = $mcount;
      $user->save();
    }
  ...
  // All Types Of Balance Update
  private function AllTypeBlnUpdate($fdate,$todate,$user_id){

    $query = Profit::select(DB::raw('SUM(amount) AS amount'))->where(['user_id'=>$user_id])
      ->whereBetween(DB::raw("DATE_FORMAT(created_at,'%Y-%m-%d')"), [$fdate, $todate])
      ->where(['iscount'=>0])
      ->where('status','!=',6)
      ->first();
  ...
  private function OpeningBlnUpdate($fdate,$todate,$user_id){
    $query = Profit::select(DB::raw('SUM(amount) AS amount'))->where(['user_id'=>$user_id])
      ->whereBetween(DB::raw("DATE_FORMAT(created_at,'%Y-%m-%d')"), [$fdate, $todate])
      ->where(['status'=> 1, 'iscount'=>0])
      ->first();
    $amount = $query->amount;
    ...
    DB::table('profits')->where(['status'=> 1, 'user_id' => $user_id])->update(['iscount' => 1]);

  }

  // DS Balance Update
  private function DSBlnUpdate($fdate,$todate,$user_id){
    $query = Profit::select(DB::raw('SUM(amount) AS amount'))->where(['user_id'=>$user_id])
      ->whereBetween(DB::raw("DATE_FORMAT(created_at,'%Y-%m-%d')"), [$fdate, $todate])
      ->where(['status'=> 2, 'iscount'=>0])
      ->first();
    $amount = $query->amount;
    ...
    DB::table('profits')->where(['status'=> 2, 'user_id' => $user_id])->update(['iscount' => 1]);

  }

  // IDS Balance Update
  private function IDSBlnUpdate($fdate,$todate,$user_id){
    $query = Profit::select(DB::raw('SUM(amount) AS amount'))->where(['user_id'=>$user_id])
      ->whereBetween(DB::raw("DATE_FORMAT(created_at,'%Y-%m-%d')"), [$fdate, $todate])
      ->where(['status'=> 3, 'iscount'=>0])
      ->first();
    $amount = $query->amount;
    ...
    DB::table('profits')->where(['status'=> 3, 'user_id' => $user_id])->update(['iscount' => 1]);

  }

  // MT Balance Update
  private function MTBlnUpdate($fdate,$todate,$user_id){
    $query = Profit::select(DB::raw('SUM(amount) AS amount'))->where(['user_id'=>$user_id])
      ->whereBetween(DB::raw("DATE_FORMAT(created_at,'%Y-%m-%d')"), [$fdate, $todate])
      ->where(['status'=> 4, 'iscount'=>0])
      ->first();
    $amount = $query->amount;
    ...
    DB::table('profits')->where(['status'=> 4, 'user_id' => $user_id])->update(['iscount' => 1]);
  }

  

  // Rank Update
  private function RankUpdate($user_id){

    $query = User::select('bmcount')->where(['id'=>$user_id])->first();
    $bmcount = $query->bmcount;

    if ($bmcount > 9 && $bmcount < 50) {
      $user = User::find($user_id);
      $user->userrank_id = 1;
      $user->save();
    } elseif($bmcount > 49 && $bmcount < 100) {
      $user = User::find($user_id);
      $user->userrank_id = 2;
      $user->save();
    } elseif($bmcount > 99 && $bmcount < 500) {
      $user = User::find($user_id);
      $user->userrank_id = 3;
      $user->save();
    } elseif($bmcount > 499 && $bmcount < 1000) {
      $user = User::find($user_id);
      $user->userrank_id = 4;
      $user->save();
    } elseif($bmcount > 999 && $bmcount < 2000) {
      $user = User::find($user_id);
      $user->userrank_id = 5;
      $user->save();
    } elseif($bmcount > 1999 && $bmcount < 5000) {
      $user = User::find($user_id);
      $user->userrank_id = 6;
      $user->save();
    } elseif($bmcount > 4999 && $bmcount < 10000) {
      $user = User::find($user_id);
      $user->userrank_id = 7;
      $user->save();
    } elseif($bmcount > 9999 && $bmcount < 100000) {
      $user = User::find($user_id);
      $user->userrank_id = 8;
      $user->save();
    }else{
    ...
    self::RankForSelf($user_id);
    
  }


  private function RankForSelf($user_id){
  ...
  private function GenCheckStep1($yearMonth){
    $count = Generation::select('id')->where('my',$yearMonth)->where(['status'=>1,'rowstatus'=>1])->count();
    
    if ($count < 1) {
      $data1['my'] = $yearMonth;
      Generation::create($data1);
    }
    $query = Generation::select('id')->where('my',$yearMonth)->where(['status'=>1,'rowstatus'=>1])->first();
    $gid = $query->id;

    $count = Vcfund::select('id')->where(['status'=>2])->where(DB::raw("DATE_FORMAT(created_at,'%Y-%m')"),$yearMonth)->count();

    if ($count < 1) {
      $ta = 0;
    }else{
      $query = Vcfund::select(DB::raw('SUM(amount) AS ta'))->where(['status'=>2])->where(DB::raw("DATE_FORMAT(created_at,'%Y-%m')"),$yearMonth)->first();
      $ta = $query->ta;
    }
    ...
    return $count = Generation::select('id')->where('my',$yearMonth)->where(['status'=>1,'rowstatus'=>1])->count();


  }


  private function IncCheckStep1($yearMonth){
    $count = Generation::select('id')->where('my',$yearMonth)->where(['status'=>1,'rowstatus'=>2])->count();
    ...
    $query = Generation::select('id')->where('my',$yearMonth)->where(['rowstatus'=>2])->first();
    $gid = $query->id;

    $count = Vcfund::select('id')->where(['status'=>1])->where(DB::raw("DATE_FORMAT(created_at,'%Y-%m')"),$yearMonth)->count();

    if ($count < 1) {
      $ta = 0;
    }else{
      $query = Vcfund::select(DB::raw('SUM(amount) AS ta'))->where(['status'=>1])->where(DB::raw("DATE_FORMAT(created_at,'%Y-%m')"),$yearMonth)->first();
      $ta = $query->ta;
    }
    ...
    return $count = Generation::select('id')->where('my',$yearMonth)->where(['status'=>1,'rowstatus'=>2])->count();


  }


  public function monthWiseGenerationUpdate(Request $request)
  {
    $rules  =  array(
      'mydate' => 'required'
    );
    ...
    $yearMonth = $request->mydate;

    $date1 = strtotime($yearMonth);
    $date2 = strtotime(date('Y-m'));

    $timestamp = $date2 - $date1;
    $dateCount =  ($timestamp)/86400;



    if ($dateCount < 1) {
      $returndata = ["Please select before month of current month"];
      return response()->json($returndata, 400,[],JSON_PRETTY_PRINT);
    }
    ...
    $fn_transaction = function() use ($request)
    {
      try
      {
        ...
        $count = Self::GenCheckStep1($request->mydate);
        if ($count < 1) {
          self::GenCheckStep2($request->mydate);
        }

        $count = Self::IncCheckStep1($request->mydate);
        if ($count < 1) {
          self::IncCheckStep2($request->mydate);
        }
  ...
  private function IncCheckStep2($yearMonth){
    ...
    $query = Generation::select('id','ta')->where('my',$yearMonth)->where(['rowstatus'=>2])->first();
    $gid = $query->id;
    $ta = $query->ta;
    ...
    if ($count > 0) {
      ...
      foreach ($incentives as $user) {
        ...
        $incid = $user->id;
        $affected = DB::table('incentives')->where(['id'=>$incid])->update(['amount' => $amount,'iscount'=>1]);
  ...
  private function GenCheckStep2($yearMonth){

    $dta[] = 0;

    $query = Generation::select('id','ta')->where('my',$yearMonth)->where(['rowstatus'=>1])->first();
    $gid = $query->id;
    $ta = $query->ta;
    ...
    $ic = $query->incentive;
    $ica = ($ta*$ic)/100;
    $count = User::select('id')->whereBetween('bmcount', [10, 49])->count();

    if ($count > 0) {
      $dta[] =+ $ica;
      $amount = $ica/$count;
      $users = User::select('id')->whereBetween('bmcount', [10, 49])->get();
    ...
    $ic = $query->incentive;
    $ica = ($ta*$ic)/100;
    $count = User::select('id')->whereBetween('bmcount', [50, 99])->count();

    if ($count > 0) {
      $dta[] =+ $ica;
      $amount = $ica/$count;
      $users = User::select('id')->whereBetween('bmcount', [50, 99])->get();
    ...
    $ic = $query->incentive;
    $ica = ($ta*$ic)/100;
    $count = User::select('id')->whereBetween('bmcount', [100, 499])->count();

    if ($count > 0) {
      $dta[] =+ $ica;
      $amount = $ica/$count;
      $users = User::select('id')->whereBetween('bmcount', [100, 499])->get();
    ...
    $ic = $query->incentive;
    $ica = ($ta*$ic)/100;
    $count = User::select('id')->whereBetween('bmcount', [500, 999])->count();

    if ($count > 0) {
      $dta[] =+ $ica;
      $amount = $ica/$count;
      $users = User::select('id')->whereBetween('bmcount', [500, 999])->get();
    ...
    $ic = $query->incentive;
    $ica = ($ta*$ic)/100;
    $count = User::select('id')->whereBetween('bmcount', [1000, 1999])->count();

    if ($count > 0) {
      $dta[] =+ $ica;
      $amount = $ica/$count;
      $users = User::select('id')->whereBetween('bmcount', [1000, 1999])->get();
    ...
    $ic = $query->incentive;
    $ica = ($ta*$ic)/100;
    $count = User::select('id')->whereBetween('bmcount', [2000, 4999])->count();

    if ($count > 0) {
      $dta[] =+ $ica;
      $amount = $ica/$count;
      $users = User::select('id')->whereBetween('bmcount', [2000, 4999])->get();
    ...
    $ic = $query->incentive;
    $ica = ($ta*$ic)/100;
    $count = User::select('id')->whereBetween('bmcount', [5000, 9999])->count();

    if ($count > 0) {
      $dta[] =+ $ica;
      $amount = $ica/$count;
      $users = User::select('id')->whereBetween('bmcount', [5000, 9999])->get();
    ...
    $ic = $query->incentive;
    $ica = ($ta*$ic)/100;
    $count = User::select('id')->whereBetween('bmcount', [10000, 100000])->count();

    if ($count > 0) {
      $dta[] =+ $ica;
      $amount = $ica/$count;
      $users = User::select('id')->whereBetween('bmcount', [10000, 100000])->get();
  ...
  private function GenBlnUpdate($fdate,$todate,$user_id){

    $query = Profit::select(DB::raw('SUM(amount) AS amount'))->where(['user_id'=>$user_id])
        ->whereBetween(DB::raw("DATE_FORMAT(created_at,'%Y-%m-%d')"), [$fdate, $todate])
        ->where(['status'=> 5, 'iscount'=>0])
        ->first();
    $amount = $query->amount;
    ...
    DB::table('profits')->where(['status'=> 5, 'user_id' => $user_id])->update(['iscount' => 1]);
  ...
  public function binaryMatchingReport(Request $request){
  ...
  private function dailyIncentiveCount($id){
    
    $user = User::select('id','userrank_id','bmcount')->where('id',$id)->first();
    //=======
    $user_id = $user->id;
    $userrank_id = $user->userrank_id;
    $bmcount = $user->bmcount;

    if ($bmcount > 9) {
      $count = Incentive::select('id')->where(['user_id'=>$user_id,'userrank_id'=>$userrank_id])->count();
      if ($count == 0) {
        $data['user_id'] = $user_id;
        $data['userrank_id'] = $userrank_id;
        $data['bmcount'] = $bmcount;
