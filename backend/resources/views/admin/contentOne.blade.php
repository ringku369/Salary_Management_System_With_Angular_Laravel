...
<form class="form-horizontal" method="POST" action="{{ route('admin.contentOneSearch.store') }} " autocomplete="off" enctype="multipart/form-data">
...
<div class="panel panel-white no-radius">
                                    <div class="panel-body">
                                        <div class="partition-light-grey padding-15 text-center margin-bottom-20">
                                            <h4 class="no-margin">Subscription List</h4>
...
{{-- <span style="cursor: pointer;" class="text-info" data-toggle="modal" data-target="#{{'brandUpdateModal'. $element['id']}}">
...
value="{{$element['id']}}"
action="{{route('admin.contentOne.delete',$element['id'])}}" 
method="delete"
token ="{{ csrf_token() }}"
>
